...
def _auto_pick_mirror_surface(asm):
    """尝试在缺省时自动选择镜面表面名。

    依次使用一组启发式关键词做模糊匹配；若未命中，则回退到包含
    "mirror" 片段的表面 key，最后再退回到字典序最小的表面 key。
    """

    # 优先使用常见命名
    for hint in ("MIRROR up", "mirror up", "mirror"):
        try:
            return _auto_resolve_surface_keys(asm, hint)
        except Exception:
            pass

    # 其次：任意包含 mirror 片段的表面
    mirrorish = [k for k in asm.surfaces if "mirror" in k.lower()]
    if len(mirrorish) == 1:
        print("[main] config.yaml 未提供 mirror_surface_name，自动选择:", mirrorish[0])
        return mirrorish[0]
    elif len(mirrorish) > 1:
        pick = sorted(mirrorish)[0]
        print(
            "[main] config.yaml 未提供 mirror_surface_name，候选有多项；",
            f"暂选 {pick}（按字典序最早）。如需指定请在 config.yaml 添加 mirror_surface_name。",
        )
        return pick

    # 最后：没有 mirror 片段，就选第一个可用表面
    if asm.surfaces:
        pick = sorted(asm.surfaces)[0]
        print(
            "[main] config.yaml 未提供 mirror_surface_name，也未找到包含 'mirror' 的表面；",
            f"暂选 {pick}。如需更精准的可视化，请在 config.yaml 设置 mirror_surface_name。",
        )
        return pick

    raise ValueError("模型中未解析到任何表面，无法自动选择镜面表面名；请在 config.yaml 填写 mirror_surface_name。")
...
def _prepare_config_with_autoguess():
    ...
    if not mirror_surface_name:
        mirror_surface_name = _auto_pick_mirror_surface(asm)
    else:
        try:
